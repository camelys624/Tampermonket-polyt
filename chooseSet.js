...
$(function(){
	...
	var renderData = {
		...
		seatMap: function(data){
            ...
            // 弹出框的删除
            var $remarksContent = $("#remarksContent");
            $remarksContent.on('click', '.seat-delete', function() {
            	var eleLi = $(this).parents('li');
                SeatMAP.get(eleLi.data('seatId')).click();
                eleLi.remove();
                if($remarksContent.find("li").length == 0){
                	$("#reminderRemarks .close-shadow").click()
                }
                interaction.createOrder(true);
            });
	...
	var interaction = {
        ...
        createOrder:function(isDeleteRemark){
        	var checkedSeatArr = [];
    		var data = [];
    		var price = null;
    		var priceId = null;
    		...
    		$checkedSeat.each(function(index,item){
    			...
    			if(idMapSeat.bak){
    				idMapSeat.seatId = mapId;
    				remarkList.push(idMapSeat);
    			}
    			data.push(param);
...
//    			var remarksDom = [];
//    			remarkList.forEach(function(item){
//    				remarksDom.push("<li data-seat-id="+ item.seatId +">"+item.sf+"-"+item.site+": <span>"+ item.bak + "</span><i class='iconfont icon-shanchu seat-delete ml15'></i></li>")
//        		})
        		interaction.showRemarks(interaction.showRemarksList(remarkList), function(){
        			var _$checkedSeat = $(".selected").not(".seat-status");
        			var _data = [];
            		_$checkedSeat.each(function(index,item){
            			var _mapId = $(item).attr("id");
            			var _idMapSeat = dataBase['idMapSeat'][_mapId];
            			var _price= interaction.getOnlyAppointObj(dataBase['priceList'], "ticketPriceId", _idMapSeat.pid).price;
            			var _priceId = _idMapSeat.pid;
            			var _seat = _idMapSeat.sid;
            			var _param = {
        					"price": _price,   //票价
        		            "priceId" :_priceId,      //票价Id
        		            "seat":_seat,          //座位ID
        		            "count": "1",        //张数
        		            "actuallyPrice": _price,  //实际票价
        		            "freeTicketCount": "1"  //套票数量
        				};
            			_data.push(_param);
            		})
            		submitParam.data = _data;
        			$("#param").val(JSON.stringify(submitParam));
        			$("#form").submit();
        		});
    		}else{
...
//        	?? 不可能走的代码
    		if(ticketCount > parseInt(ticketNumber)){
    			...
    			if(parseInt(purchaseRestrictions)==parseInt(ticketNumber)){
                    alert("本场次您只能购买"+ticketNumber+"张，购买数量超出限制");
				}else{
        ...
        svgLink:function(){
            $("#rendersvg").on("dblclick","g>*:not(image,title)",function(){
    			...
    			"&showTime="+showTime+"&theaterId="+theaterId+"&productId="+productId+"&isRealName="+isRealName+"&purchaseRestrictions="+purchaseRestrictions+
    			"&ticketNumber="+ticketNumber+"&manageWayCode="+manageWayCode+"&productTypeName="+productTypeName+"&productSubtypeName="+productSubtypeName+"&threaterName="+threaterName+"&sign="+sign+"&productName="+productName;
		...
		showRemarks: function(content,callback){
			$("#reminderRemarks").show();
			$(".shadowDiv").show();
			$("#reminderRemarks #remarksContent").html(content);
			$("#reminderRemarks #continueBuy").off("click");
			$("#reminderRemarks #continueBuy").click(function(){
				callback && callback();
			})
			$("#reminderRemarks .close-shadow").off("click");
			$("#reminderRemarks .close-shadow").click(function(){
				$("#reminderRemarks").hide();
				$(".shadowDiv").hide();
			})
		},
		showRemarksList: function(remarkList){
			var remarksDom = [];	
			for(var index = 0; index < remarkList.length; index++){
				var item = remarkList[index]; 
				if(index < 9){
					remarksDom.push("<li data-seat-id="+ item.seatId +">"+item.sf+"-"+item.site+": <span>"+ item.bak + "</span><i class='iconfont icon-shanchu seat-delete ml15'></i></li>")	
				}else{
					if(remarkList.length == 10){
						remarksDom.push("<li data-seat-id="+ item.seatId +">"+item.sf+"-"+item.site+": <span>"+ item.bak + "</span><i class='iconfont icon-shanchu seat-delete ml15'></i></li>")	
					}else{
						remarksDom.push("<li> ······ 合计"+ remarkList.length +"个座位</li>");	
						break;
					}
				}
			}
